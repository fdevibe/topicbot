...
function Bot(opts) {
    ...
    function validArgs() {
        return [
            ...
            ' heilagdagar [datestring] -- Vis ei liste over heilagdagar i dag eller for valt dato',
            ...
            ' topic -- Set emne frå bursdagslista eller heilagdagslista',
            ' topic [topic] -- Set manuelt topic'
        ].join('\n');
    }
    ...
    setManualTopic = function (topic) {
        client.send('TOPIC', opts.channel, topic);
    };

    setTopic = function (topic) {
        var todaysBirthdays;
        if (topic !== undefined && topic.length > 0) {
            setManualTopic(topic.join(' '));
            return;
        }
        todaysBirthdays = getBirthdays(moment());
